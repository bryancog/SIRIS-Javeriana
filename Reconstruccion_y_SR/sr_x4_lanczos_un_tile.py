...
def main():
    ...
    parser.add_argument("--output-root", default="outputs_piloto_sr_first_x4")
    ...
    tile_path = in_dir / args.tile_name
    if not tile_path.exists():
        raise FileNotFoundError(f"No existe el tile: {tile_path}")

    tiles = [tile_path]

    log("=" * 80)
    log(f"SR x{args.scale} Lanczos fecha {args.date} — tile único")
    log(f"Entrada: {tile_path}")
    ...
    with open(out_dir / "summary_x4_tile.json", "w") as f:
