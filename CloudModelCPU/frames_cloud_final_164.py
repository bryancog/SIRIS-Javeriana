...
inp=base/"outputs_cloud_scl_real/final_164_cloud_imputed_sr_x4"
out=base/"outputs_cloud_scl_real/final_164_vis"
frames=out/"frames_png"
frames.mkdir(parents=True,exist_ok=True)

files=sorted(inp.glob("date_*/tile_r00000_c01536_cloud_imputed_x4.npy"))
...
for i,p in enumerate(files):
    ...
    draw.rectangle((10,10,310,55),fill=(0,0,0))
    draw.text((25,25),fecha,fill=(255,255,255))

    outp=frames/f"frame_{i:04d}_{fecha}.png"
    img.save(outp,optimize=True)

    if i==0 or (i+1)%20==0 or i==len(files)-1:
        print(f"{i+1}/{len(files)} {outp}",flush=True)
