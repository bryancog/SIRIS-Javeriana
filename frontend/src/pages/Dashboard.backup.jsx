...
export default function Dashboard() {
  ...
  const [resultsHint, setResultsHint] = useState("Usa la herramienta de polÃ­gono para generar resultados.");
  const [isHintError, setIsHintError] = useState(false);
  const [areaStatus, setAreaStatus] = useState(`Ãrea mÃ¡xima permitida: ${MAX_AREA_KM2} kmÂ²`);
  ...
  function initializeMap() {
    ...
    map.on(L.Draw.Event.CREATED, (event) => {
      ...
      if (polygonAreaM2 > MAX_AREA_M2) {
        clearResults(`El polÃ­gono supera el mÃ¡ximo permitido de ${MAX_AREA_KM2} kmÂ².`);
        setAreaStatus(`Ãšltima selecciÃ³n: ${formatAreaKm2(polygonAreaM2)} kmÂ² | MÃ¡ximo: ${MAX_AREA_KM2} kmÂ²`);
        return;
      }
  ...
  async function loadStudyArea(map) {
    try {
      ...
      console.error("Error cargando Ã¡rea de estudio", error);
    }
  }

  function setExportProgress(message, state = "running") {
    setProgress({ visible: true, message: message || "Procesando exportaciÃ³n...", state });
  }
  ...
  function clearResults(message) {
    ...
    setResultsHint(message || "Usa la herramienta de polÃ­gono para generar resultados.");
    setIsHintError(Boolean(message));
    setAreaStatus(`Ãrea mÃ¡xima permitida: ${MAX_AREA_KM2} kmÂ²`);
  }
  ...
  async function exportSelectedArea(layer, polygonAreaM2) {
    ...
    setResultsHint("Generando imÃ¡genes y video del Ã¡rea seleccionada...");
    setResultsCount("Procesando");
    setAreaStatus(`Ãrea seleccionada: ${formatAreaKm2(polygonAreaM2)} kmÂ²`);
    ...
    try {
      ...
      stopExportProgressPolling();
      setExportProgress("ExportaciÃ³n finalizada.", "done");
      setResultsHint("ExportaciÃ³n generada correctamente.");
      setResultsCount("1 video");
      setResults([
        {
          title: "Ãrea seleccionada",
          description: "Video, GeoTIFF y CSV generados para el Ã¡rea solicitada.",
      ...
      stopExportProgressPolling();
      setExportProgress("Error generando exportaciÃ³n.", "error");
  ...
  async function cancelActiveExport() {
    try {
      ...
      stopExportProgressPolling();
      setExportProgress("ExportaciÃ³n cancelada.", "error");
    } catch (error) {
      console.error("No se pudo cancelar la exportaciÃ³n activa", error);
    }
  }
    ...
    <main className="dashboard">
      <aside className="sidebar">
        <div className="sidebar-header">
          <span className="brand-kicker">SesiÃ³n activa</span>
          <h2>Resultados Sentinel-2</h2>
          <button className="ghost-button" type="button" onClick={handleLogout}>Cerrar sesiÃ³n</button>
          <p>Dibuja un polÃ­gono en el mapa para generar productos que intersectan tu zona de interÃ©s.</p>
        </div>

        <div className="status-card">
          <span className="status-label">RegiÃ³n enfocada</span>
          <strong>PacÃ­fico Colombiano</strong>
        ...
        {progress.visible && (
          <div className={`export-progress-box is-${progress.state}`}>
            <div className="export-progress-title">Estado de la exportaciÃ³n</div>
      ...
      <section className="map-section">
        <div className="map-toolbar">
          <div>
            ...
            <p>Traza un polÃ­gono y revisa la generaciÃ³n de productos satelitales.</p>
          </div>
        </div>
