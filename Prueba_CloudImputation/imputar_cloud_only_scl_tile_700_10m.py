...
out=base/"outputs_cloud_scl_real/tile_r00000_c01536_cloud_only_10m_700"
out.mkdir(parents=True,exist_ok=True)

files=sorted(root.glob("date_*/bands_scl.npy"))
...
log(f"working shape: {working.shape}")
log(f"cloud_fraction_global: {cloud_mask.mean():.4f}")
log(f"nan_before_working: {np.isnan(working).mean():.4f}")
...
# preservar originales no nubosos; reemplazar solo nubes
final=bands.copy()
for t in range(T):
    m=cloud_mask[t]
    for b in range(4):
...
for t,fecha in enumerate(fechas):
    ...
    np.save(od/"tile_r00000_c01536_cloud_only_10m.npy", final[t])
...
summary={
    ...
    "sr_applied":False,
    "cloud_fraction_global":float(cloud_mask.mean()),
    "nan_original_bands":float(np.isnan(bands).mean()),
    "nan_working_after_cloud_mask":float(np.isnan(working).mean()),
    "nan_final":float(np.isnan(final).mean()),
    "note":"Only pixels classified as cloud/shadow/cirrus by SCL were replaced. Non-cloud pixels were preserved."
}
