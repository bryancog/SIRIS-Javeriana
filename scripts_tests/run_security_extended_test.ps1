...
$scriptPath = Join-Path $ProjectRoot "scripts_tests\run_security_extended_test.py"
$evidenceDir = Join-Path $ProjectRoot "tests_evidence\security"
$logPath = Join-Path $evidenceDir "security_extended_$timestamp.log"
...
Write-Host "==================================================="
Write-Host "SIRIS - Seguridad funcional ampliada v0.7.1"
...
if ($SkipDbCheck) {
  & $pythonExe $scriptPath `
    ...
    --skip-db-check 2>&1 | Tee-Object -FilePath $logPath -Append
} else {
  & $pythonExe $scriptPath `
    --base-url $BaseUrl `
    --project-root $ProjectRoot `
